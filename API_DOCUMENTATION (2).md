...
- [Shipping (RajaOngkir)](#-shipping-rajaongkir)
  - [Get Provinces](#1-get-provinces)
  - [Get Cities](#2-get-cities)
  - [Get Districts](#3-get-districts)
  - [Search Destination](#4-search-destination)
  - [Calculate Shipping](#5-calculate-shipping)
...
| Role | Deskripsi |
| ---- | --------- |
| `customer` | User biasa — bisa membeli DAN menjual produk (unified buyer/seller) |
| `admin` | Administrator — mengelola seluruh platform |

> 💡 **Tidak ada role `seller` terpisah.** Semua customer secara otomatis bisa berjualan (membuat produk, melihat dashboard penjualan, dll).
...
>
> 💡 Semua user yang login (customer maupun admin) bisa membuat, mengubah, dan menghapus **produknya sendiri**.
...
> 💡 **Semua user yang login** bisa membuat produk — tidak perlu role `seller` khusus.
...
| weight      | number | ❌        | Berat produk dalam gram (default: 1000)  |
...
Memperbarui data produk. Hanya **pemilik produk** yang bisa mengubah.
> 💡 **Semua user yang login** bisa mengelola produknya sendiri.
...
Menghapus produk secara permanen. Hanya **pemilik produk** yang bisa menghapus.
> 💡 **Semua user yang login** bisa menghapus produknya sendiri.
...
| shipping_cost     | number | ❌        | Ongkos kirim (dari perhitungan RajaOngkir) |
| courier           | string | ❌        | Kode kurir (jne, pos, tiki, dsb)          |
| shipping_service  | string | ❌        | Layanan kurir (REG, YES, dsb)             |
| shipping_address  | object | ❌        | Alamat pengiriman (format bebas)          |
| notes            | string | ❌        | Catatan tambahan untuk pesanan            |

> 💡 **Info:** Harga per produk dan total harga (total_amount) **dihitung secara otomatis oleh server** (server-side) berdasarkan harga yang tersimpan di database untuk keamanan. Anda tidak perlu mengirimkannya di payload.
...
| city        | string  | ✅        | Kota                              |
| city_id     | number  | ❌        | ID Kota (dari API RajaOngkir)     |
| province    | string  | ✅        | Provinsi                          |
| province_id | number  | ❌        | ID Provinsi (dari API RajaOngkir) |
...
> 🔒 Membutuhkan **Bearer Token**.
>
> 🔌 Terintegrasi dengan **RajaOngkir Komerce API** (`https://rajaongkir.komerce.id/api/v1`)
>
> ⚠️ Pastikan `RAJAONGKIR_API_KEY` sudah diisi di file `.env`
...
{
  "success": true,
  "message": "Berhasil mengambil daftar kota/kabupaten",
  "data": [
    {
      "city_id": "151",
      "province_id": "6",
...
| Parameter | Type   | Deskripsi    |
| --------- | ------ | ------------ |
| city_id   | number | ID kota       |

**Contoh Request:**

```
GET /api/shipping/districts/151
```
...
| Status | Kondisi                |
| ------ | ---------------------- |
| 400    | City ID tidak valid     |

---

### 4. Search Destination

Pencarian langsung destinasi domestik (autocomplete-style). Berguna untuk form input yang membutuhkan ID tujuan tanpa harus memilih provinsi → kota → kecamatan secara bertahap.

```
GET /api/shipping/search?q=surabaya
```
...
| limit     | number | 10      | Jumlah hasil per halaman       |
...
| Status | Kondisi                           |
| ------ | --------------------------------- |
| 400    | Parameter pencarian (q) kosong     |

---

### 5. Calculate Shipping

Menghitung ongkos kirim domestik menggunakan RajaOngkir API.
...
> ⚠️ `origin` dan `destination` harus berupa **ID kota/kecamatan** (didapat dari endpoint Get Cities / Get Districts / Search Destination). Bukan nama kota!

**Request Body:**

| Field       | Type   | Required | Deskripsi                                       |
| ----------- | ------ | -------- | ----------------------------------------------- |
| origin      | number | ✅        | ID kota/kecamatan asal                           |
| destination | number | ✅        | ID kota/kecamatan tujuan                         |
| weight      | number | ✅        | Berat paket dalam **gram** (misal: 1000 = 1 kg)  |
| courier     | string | ✅        | Kode kurir (lihat tabel di bawah)                |
...
> 💡 Kurir yang tersedia tergantung pada tipe akun RajaOngkir Anda (Starter/Basic/Pro).

**Error Responses:**

| Status | Kondisi                                          |
| ------ | ------------------------------------------------ |
| 400    | Origin / destination / weight / courier kosong    |
| 404    | Tidak ditemukan tarif untuk rute & kurir tersebut |
| 500    | API Key belum dikonfigurasi                       |
| 500    | Error dari RajaOngkir API                         |

---

### Environment Variables (Shipping)

| Variable               | Deskripsi                         | Contoh                                             |
| ---------------------- | --------------------------------- | -------------------------------------------------- |
| `RAJAONGKIR_API_KEY`   | API Key dari dashboard RajaOngkir | `abc123def456`                                     |
| `RAJAONGKIR_BASE_URL`  | Base URL API (opsional)           | `https://rajaongkir.komerce.id/api/v1` (default)   |
...
> 🔒 Membutuhkan **Bearer Token**. Data di-scope ke produk milik user yang sedang login.
>
> 💡 Semua user yang login bisa mengakses dashboard ini. Jika user belum pernah membuat produk, dashboard akan menampilkan data kosong (0).

---

### 1. Get Seller Dashboard Stats

Menampilkan statistik penjualan untuk user: pendapatan, jumlah produk, pesanan, dan produk terlaris.
...
| GET | `/api/shipping/provinces` | Daftar provinsi |
| GET | `/api/shipping/cities/:province_id` | Daftar kota per provinsi |
| GET | `/api/shipping/districts/:city_id` | Daftar kecamatan per kota |
| GET | `/api/shipping/search?q=...` | Cari destinasi (autocomplete) |
| POST | `/api/shipping/calculate` | Hitung ongkir (RajaOngkir) |
